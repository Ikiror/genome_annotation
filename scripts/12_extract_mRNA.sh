...
#thsi script will extract mRNA seq and filter the fasta file
...
#extract a list of remaining mRNA ids from the filtered gff file
grep -P "\tmRNA\t" $FILTERED_GFF | awk '{print $9}' | cut -d ';' -f1 | sed 's/ID=//g' > list.txt

#use generated list to filter the transcript and protein FASTA files 
